...
try:
    ...
    speedtest_download = str(
        driver.capturarTexto('//span[@class="result-data-large number result-data-value download-speed"]'))
    speedtest_upload = str(
        driver.capturarTexto('//span[@class="result-data-large number result-data-value upload-speed"]'))
...
try:
    ...
    sleep(60)
    minha_conexao_download = \
    str(driver.capturarTexto('/html/body/section[1]/div/div[2]/div[2]/div[7]/div/div[1]/div[2]/div/span[3]')).split(
        " ")[0]
    minha_conexao_upload = \
    str(driver.capturarTexto('/html/body/section[1]/div/div[2]/div[2]/div[7]/div/div[1]/div[3]/div/span[3]')).split(
        " ")[0]
    imagem = ImageGrab.grab()
...
try:
    ...
    for linha in range(int(quantidade_dias_mes)):
        ...
        # Coluna Dia
        sheet.cell(row=linha + 3, column=1).value = linha + 1 if sheet.cell(row=linha + 3,
                                                                            column=1).value == None else None

        if data_atual.day == sheet.cell(row=linha + 3, column=1).value:
            # Coluna Minha Conexao Download
            sheet.cell(row=linha + 3, column=2).value = minha_conexao_download.replace('.', ',') if sheet.cell(
                row=linha + 3, column=2).value == None else None

            # Coluna Minha Conexao Upload
            sheet.cell(row=linha + 3, column=3).value = minha_conexao_upload.replace('.', ',') if sheet.cell(
                row=linha + 3, column=3).value == None else None

            # Coluna Speed Test Download
            sheet.cell(row=linha + 3, column=4).value = speedtest_download.replace('.', ',') if sheet.cell(
                row=linha + 3, column=4).value == None else None

            # Coluna Speed Test Download
            sheet.cell(row=linha + 3, column=5).value = speedtest_upload.replace('.', ',') if sheet.cell(row=linha + 3,
                                                                                                         column=5).value == None else None
            ...
            if sheet.cell(row=linha + 3, column=6).value == None:
                sheet.cell(row=linha + 3,
                           column=6).value = f'=({celula_download_minha_conexao}+{celula_download_speed_teste})/2'
            if sheet.cell(row=linha + 3, column=7).value == None:
                sheet.cell(row=linha + 3,
                           column=7).value = f'=({celula_upload_minha_conexao}+{celula_upload_speed_teste})/2'
    ...
    sheet.cell(row=int(quantidade_dias_mes) + 3,
               column=6).value = f"=SUM({primeira_celula_download}:{ultima_celula_download})/{int(quantidade_dias_mes)}".upper()
    sheet.cell(row=int(quantidade_dias_mes) + 3,
               column=7).value = f"=SUM({primeira_celula_upload}:{ultima_celula_upload})/{int(quantidade_dias_mes)}".upper()
